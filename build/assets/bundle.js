(function (factory) {
  ...
  .replace(/ /g, '.')}`;
	...
	function sendForm(form) {
	  ...
	  grecaptcha.ready(function () {
	    grecaptcha.execute(GOOGLE_CAPTHCA_V_3_SITE_KEY, {
	      ...
	      console.log(token, form.querySelector('.g-recaptcha-response'), form.querySelector('.g-recaptcha-response').value);
	...
	const projectButtons$1 = document.querySelectorAll('.portfolio-single-view__grid-item-btn');
	if (projectButtons$1.length) {
	  // Находим модальное окно
	  const projectModal = document.querySelector('.project-modal'); // Убедитесь, что у вас есть модалка с таким классом

	  projectButtons$1.forEach(button => {
	...
	// Находим все кнопки .portfolio-single-view__grid-item-btn
	const projectButtons = document.querySelectorAll('.portfolio-single-view__grid-item-btn');
